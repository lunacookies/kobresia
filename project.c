...
static void
dense_init(struct dense *d, struct mem *m, smm count, smm elem_len)
{
	assert_zero(d);
	assert(count > 0);
	assert(elem_len > 0);
	...
	d->starts = alloc_u(&m->temp, s32, count + 1);
}

static struct strbuilder *
dense_push(struct dense *d)
{
	d->starts[d->count] = cast(s32) d->data.used;
	d->count++;
	return &d->data;
}

static void
dense_finish(struct dense *d, struct mem *m, char **data, s32 **starts)
{
	d->starts[d->count] = cast(s32) d->data.used;
	...
	*starts = alloc_copy(&m->perm, s32, d->starts, d->count + 1);
}
...
project_search(struct project *p, struct mem *m)
{
	...
	s32 *file_pkgs = alloc_u(&m->temp, s32, MAX_FILES);

	s32 *pkg_first_files = alloc_u(&m->temp, s32, MAX_PKGS);
	s32 *pkg_file_counts = alloc_u(&m->temp, s32, MAX_PKGS);
	...
	while (true) {
		...
		while (true) {
			...
			file_pkgs[file_count] = cast(s32) pkg_count;
		...
		pkg_first_files[pkg_count] = cast(s32) first_file_in_pkg;
		pkg_file_counts[pkg_count] = cast(s32) files_in_pkg;
	...
	p->file_pkgs = alloc_copy(&m->perm, s32, file_pkgs, file_count);

	p->pkg_first_files =
	        alloc_copy(&m->perm, s32, pkg_first_files, pkg_count);
	p->pkg_file_counts =
	        alloc_copy(&m->perm, s32, pkg_file_counts, pkg_count);

	arena_temp_end(t);
}

struct str
project_file_name(struct project *p, smm id)
{
	assert(id < p->file_count);
	smm start = cast(smm) p->file_name_starts[id];
	smm end = cast(smm) p->file_name_starts[id + 1];
	assert(end >= start);
...
struct str
project_file_path(struct project *p, smm id)
{
	assert(id < p->file_count);
	smm start = cast(smm) p->file_path_starts[id];
	smm end = cast(smm) p->file_path_starts[id + 1] -
...
struct str
project_pkg_name(struct project *p, smm id)
{
	assert(id < p->pkg_count);
	smm start = cast(smm) p->pkg_name_starts[id];
	smm end = cast(smm) p->pkg_name_starts[id + 1];
	assert(end >= start);
	smm length = end - start;
...
struct str
project_pkg_path(struct project *p, smm id)
{
	assert(id < p->pkg_count);
	smm start = cast(smm) p->pkg_path_starts[id];
	smm end = cast(smm) p->pkg_path_starts[id + 1] -
	          1; // remove null terminator
	assert(end >= start);
	smm length = end - start;
