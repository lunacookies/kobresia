...
main(s32 argc, char **argv)
{
	...
	for (smm pkg_id = 0; pkg_id < proj.pkg_count; pkg_id++) {
		struct str name = project_pkg_name(&proj, pkg_id);
		struct str path = project_pkg_path(&proj, pkg_id);

		printf("\n");
		printf("id:   %td\n", pkg_id);
		printf("name: (%td) %.*s\n", name.n, cast(s32) name.n, name.p);
		printf("path: (%td) %.*s\n", path.n, cast(s32) path.n, path.p);

		smm file_count = proj.pkg_file_counts[pkg_id];
		smm first_file = proj.pkg_first_files[pkg_id];
		smm end = first_file + file_count;

		for (smm file_id = first_file; file_id < end; file_id++) {
			...
			printf("\tname: (%td) %.*s\n", file_name.n,
			        cast(s32) file_name.n, file_name.p);
			printf("\tpath: (%td) %.*s\n", file_path.n,
			        cast(s32) file_path.n, file_path.p);
		}
	}
